...
import stylesCss from 'themes/halo-box/styles.css';
...
window.Vaadin['_vaadintheme_halo-box_globalCss'] = window.Vaadin['_vaadintheme_halo-box_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_halo-box_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_halo-box_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_halo-box_globalCss'].push(target);
  }
  if (!document['_vaadintheme_halo-box_componentCss']) {
    
    document['_vaadintheme_halo-box_componentCss'] = true;
  }
